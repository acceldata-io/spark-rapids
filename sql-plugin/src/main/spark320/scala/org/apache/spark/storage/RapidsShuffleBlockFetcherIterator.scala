...
final class RapidsShuffleBlockFetcherIterator(
  ...
  private[this] val pushBasedFetchHelper = new RapidsPushBasedFetchHelper(
    ...
    blockId match {
      case ShuffleBlockId(shufId, mapId, reduceId) =>
        ...
        throw new FetchFailedException(
          address, shufId, mapId, mapIndex, reduceId, msg, e)
      case ShuffleBlockBatchId(shuffleId, mapId, startReduceId, _) =>
        // from SparkCoreErrors
        throw new FetchFailedException(
          address, shuffleId, mapId, mapIndex, startReduceId, msg, e)
      case _ =>
        // from SparkCoreErrors
        throw new SparkException(s"Failed to get block $blockId, which is not a shuffle block", e)
    }
  }
...
private class RapidsBufferReleasingInputStream(
    ...
    private val currentResult: RapidsShuffleBlockFetcherIterator.SuccessFetchResult,
    ...
    try {
      ...
      case e: IOException if detectCorruption =>
        ...
        try {
          ...
          case _: Throwable =>
          // like: IOUtils.closeQuietly(this)
          // adding like this since it is deprecated
        }
...
object RapidsShuffleBlockFetcherIterator {
  ...
  def makeIterator(
    ...
    new RapidsShuffleBlockFetcherIterator(
      ...
      sparkEnv.conf.get(config.SHUFFLE_CHECKSUM_ALGORITHM),
      readMetrics,
