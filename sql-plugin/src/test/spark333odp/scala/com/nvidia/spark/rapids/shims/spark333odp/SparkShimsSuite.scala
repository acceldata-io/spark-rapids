...
class SparkShimsSuite extends AnyFunSuite with FQSuiteName {
  test("spark shims version") {
    assert(ShimLoader.getShimVersion === AcceldataShimVersion(3, 3, 3, "3.3.6.501-SNAPSHOT"))
  }

  test("shuffle manager class") {
    assert(ShimLoader.getRapidsShuffleManagerClass ===
      classOf[com.nvidia.spark.rapids.spark333odp.RapidsShuffleManager].getCanonicalName)
  }
